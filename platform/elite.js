...
class DemoCard {
  ...
  set_guide(guide) {
    ...
    this.set_description(desc_content);
  }

  set_error(extra_info) {
    let desc_content = "";
    desc_content += `<div class='error-div'> ${ERROR_SVG} </div>`;
    desc_content += `<div class='error-text'> ${common_translations['demo_error']}. </div>`;
    desc_content += `<div class='error-code'> ${common_translations['error_code']}: </div>`;
    desc_content += `<div class='error-details'> ${extra_info} </div>`;
    ...
    if (state === 'error') {
      ...
      this.set_error(extra_state);
			this.card.classList.add('card-error');
