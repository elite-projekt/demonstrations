...
default_email_account_password = "123" # nosec
...
# sender_email = "test2@example.org" #"max.mustermann@mpseinternational.com"
# #test2@example.org
# receiver_email = "max.mustermann@mpseinternational.com"
# email_password = "123"

# email_client_config_location = \
#     r"C:\Users\Jan\AppData\Roaming\Thunderbird\Profiles\sldrcrlj.MPSE\prefs.js"
email_client_config_location = (
        os.getenv("APPDATA") + r"\Thunderbird\Profiles\sldrcrlj.MPSE\prefs.js"
)
...
def create_mail(
    ...
    part2 = email.mime.text.MIMEText(html_from_file, "html", "utf-8")
...
def send_mail(
        sender, password, port, recipient, message,
...
def send_multiple_mails_from_files(
        local_password, port, local_server=default_email_server
):
    for file in glob.glob(email_files_location + "*.txt"):
        ...
        # If the form '"name" <name@domain>' is used the raw
...
def change_client_profile(use_secured_client=True, change_ports=False):
    """
    Changes the client configuration between a more secure and a less secure
    configuration
    ...
    with open(email_client_config_location, "r") as f:
        ...
        # only touch port configuration if we really want to (multiple email
        # servers with different ports are not in scope of the WS20/21
        if change_ports:
            i = 0
            imap_port_set = False
            smtp_port_set = False
            while i < len(f_lines):
                if "mail.server.server1.port" in f_lines[i] \
                    ...
                    f_lines[i] = f_lines[i].replace(
                        str(unsecure_server_imap_port),
                        str(secure_server_imap_port)
                    )
                    imap_port_set = True
                elif (
                    ...
                    imap_port_set = True
                elif "mail.smtpserver.smtp1.port" in f_lines[i] \
                        and use_secured_client:
                    f_lines[i] = f_lines[i].replace(
                        str(unsecure_server_smtp_port),
                        str(secure_server_smtp_port)
                    )
                    smtp_port_set = True
                elif (
        ...
        while i < len(f_lines):
            if "mail.phishing.detection.enabled" in f_lines[i] \
                    and use_secured_client:
        ...
        if not remote_image_set:
            if use_secured_client:
                f_lines.append(
                    'user_pref("mailnews'
                    '.message_display.disable_remote_image", true);\n'
                )
            elif not use_secured_client:
                f_lines.append(
                    'user_pref("mailnews'
                    '.message_display.disable_remote_image", false);\n'
                )
...
# f = open("email.txt", "w")
# a = create_mail(subject="Test2", sender=default_email_account,
# recipient="test2@example.org")
# f.write(a.as_string()) # create_mail(subject="Test2", sender=sender_email,
# recipient="test2@example.org").as_string())
# f.close()

delete_mailbox()
send_multiple_mails_from_files(default_email_account_password,
                               secure_server_smtp_port)

# f = open("email.txt", "r")
# mail_message = f.read()
# send_mail(sender="test2@example.org", password=email_password,
# port=secure_server_smtp_port, recipient=receiver_email, message=mail_message,
# server=email_server)
